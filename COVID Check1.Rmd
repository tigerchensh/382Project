...
```{r}
death <- count(cov, cov$death_yn)
barplot(death$n, names.arg=death$`cov$death_yn`,xlab="Case death status", ylab="Count of number",main="Number of Covid cases and death status")
```

```{r}
caseDeath <- cov[cov$death_yn=='Yes',]
caseDeath_date <- count(caseDeath, caseDeath$cdc_report_dt)
caseDeath_date$date <- as.Date(caseDeath_date$`caseDeath$cdc_report_dt`)
caseDeath_date$date <- caseDeath_date$date - as.Date("2020-01-01")
caseDeath_date$cum_case <- cumsum(caseDeath_date$n)
```

```{r}
plot(caseDeath_date$date,caseDeath_date$n,xlab="Case report date to CDC (0 as first date 2020-01-01)",ylab="Covid death case", main=" Covid death case number and CDC report date")
```

There is a moderate positive linear relationship between CDC report date and Covid death case number.


```{r}
plot(caseDeath_date$date,caseDeath_date$cum_case,xlab="Case report date to CDC (0 as first date 2020-01-01)",ylab="culmulative Covid death case", main=" Culmulative Covid death case and CDC report date")
```

There is a strong positive nonlinear relationship between CDC report date and culmulative Covid death case number.
